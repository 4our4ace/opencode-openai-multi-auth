...
export function normalizeModel(model) {
    ...
    if (normalized.includes("gpt-5.3-codex-max") ||
        normalized.includes("gpt 5.3 codex max")) {
        return "gpt-5.3-codex-max";
    }
    // 2. GPT-5.3 Codex Mini
    if (normalized.includes("gpt-5.3-codex-mini") ||
...
function resolveInclude(modelConfig, body) {
    ...
    const base = body.include ??
        ...
        modelConfig.include ?? ["reasoning.encrypted_content"];
...
export function getReasoningConfig(modelName, userConfig = {}) {
    ...
    const isGpt52General = (normalizedName.includes("gpt-5.2") ||
    ...
    const isNewerGptFamily = Boolean(gptVersion) &&
        (gptMajor > 5 || (gptMajor === 5 && gptMinor >= 3));
    // GPT 5.2, GPT 5.2 Codex, and Codex Max support xhigh reasoning
    const supportsXhigh = isGpt52General || isGpt52Codex || isCodexMax || isNewerGptFamily;
    ...
    const supportsNone = isGpt52General || isGpt51General || (isNewerGptFamily && !isCodex);
 ...
 */
export async function transformRequestBody(body, userConfig = { global: {}, models: {} }, codexMode = true) {
