...
import { registerProviderAuthResolver, } from "./lib/provider-auth-resolvers.js";
import { SessionBindingStore } from "./lib/session-bindings.js";
export function createOpenAIProviderAuthResolver() {
    return async ({ providerID, sessionID }) => {
        ...
        const account = boundAccountIndex === undefined
...
const tui = async (api) => {
    ...
    const sessionBindings = new SessionBindingStore();
    sessionBindings.loadFromDisk();
    const unregisterProviderAuthResolver = registerProviderAuthResolver("openai", createOpenAIProviderAuthResolver());
