...
describe("Request Transformer Module", () => {
  describe("normalizeModel", () => {
    ...
    describe("Codex CLI preset names", () => {
      ...
      it("should normalize codex mini presets to gpt-5-codex-mini", async () => {
        ...
        expect(normalizeModel("gpt-5-codex-mini-medium")).toBe(
          "gpt-5-codex-mini",
        );
        expect(normalizeModel("gpt-5-codex-mini-high")).toBe(
          "gpt-5-codex-mini",
        );
        expect(normalizeModel("openai/gpt-5-codex-mini-high")).toBe(
          "gpt-5-codex-mini",
        );
        expect(normalizeModel("codex-mini-latest")).toBe("gpt-5-codex-mini");
        expect(normalizeModel("openai/codex-mini-latest")).toBe(
          "gpt-5-codex-mini",
        );
      });
    ...
    describe("Edge cases", () => {
      ...
      it("should handle old verbose names", async () => {
        ...
        expect(normalizeModel("GPT 5 High (ChatGPT Subscription)")).toBe(
          "gpt-5",
        );
      });
  ...
  describe("addCodexBridgeMessage", () => {
    it("should prepend bridge message when tools present", async () => {
      ...
      expect(result![0].role).toBe("developer");
      expect(result![0].type).toBe("message");
      expect((result![0].content as any)[0].text).toContain(
  ...
  describe("transformRequestBody", () => {
    it("preserves existing prompt_cache_key passed by host (OpenCode)", async () => {
      ...
      const result: any = await transformRequestBody(body);
    ...
    it("should apply user reasoning config", async () => {
      ...
      const result = await transformRequestBody(body, userConfig);
    ...
    describe("CODEX_MODE parameter", () => {
      it("should use bridge message when codexMode=true and tools present (default)", async () => {
        ...
        const result = await transformRequestBody(body, undefined, true);

        expect(result.input).toHaveLength(2);
        expect(result.input![0].role).toBe("developer");
        expect((result.input![0].content as any)[0].text).toContain(
          "Codex Running in OpenCode",
        );
      });
      ...
      it("should use tool remap message when codexMode=false", async () => {
        ...
        const result = await transformRequestBody(body, undefined, false);
